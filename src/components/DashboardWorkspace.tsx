/**
 * DashboardWorkspace — Main content area for authenticated users.
 * Layout (default/overview tab):
 *   1. Region selector pill  ← NEW position (above search bar)
 *   2. Hero search bar       ← Full width now
 *   3. Terminal log
 *   4. Results display
 */
        ...
        return (
          <div className="space-y-6 animate-in fade-in duration-500">
            ...
            <div className="text-center space-y-3">
              ...
              >
                DEPLOY{' '}
                <span className="text-hunter-cyan">HUNTING</span>{' '}
                NODES
              </motion.h2>
              <p className="text-hunter-muted font-mono text-xs uppercase tracking-[0.3em]">
                Neural link stable. Awaiting target parameters.
              </p>
            </div>

            {/* ── REGION SELECTOR — full width pill ABOVE the search bar ── */}
            <RegionSelector
            ...
            {/* ── SEARCH BAR — now full width with no region selector inside ── */}
            <HeroSearchBar
              query={query}
            ...
            {/* Verifier offline banner */}
            {status === SearchStatus.VERIFIER_OFFLINE && (
              <motion.div
                initial={{ opacity: 0, y: 10 }}
                animate={{ opacity: 1, y: 0 }}
                className="flex items-center gap-4 bg-red-950/40 border border-red-500/40
                           rounded-xl p-4 shadow-[0_0_20px_rgba(239,68,68,0.1)]"
              >
                <WifiOff className="text-red-400 shrink-0" size={20} />
                <div>
                  <div className="text-red-400 font-display font-bold text-sm uppercase tracking-wider">
                    Verification Network Offline
                  </div>
                  <p className="text-red-400/60 text-xs font-mono mt-0.5">
                    Our checkout verification service is temporarily unavailable.
                    No codes can be shown until it comes back online — this protects you
                    from receiving unverified codes.
                  </p>
                </div>
              </motion.div>
            )}
